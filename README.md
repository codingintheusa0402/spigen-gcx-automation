...
│   ├── ChatRoomVisitorLog/              # Google Chat room visitor log (joined / left / posted) → Sheet
...
| [GAS_Operations/ChatRoomVisitorLog](GAS_Operations/ChatRoomVisitorLog/) | Logs who visits the GCX Google Chat rooms — polls the Chat API every 10 min and appends `BASELINE` / `JOINED` / `LEFT` / `POSTED` rows (user, email, space, timestamp) to a `Visitor_Log` sheet, keeps a current `Members` roster per room. User-auth, no bot required. | [README](GAS_Operations/ChatRoomVisitorLog/README.md) |
